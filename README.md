...
If the network environment (Wi-Fi) changes, you MUST update the IP addresses in the following files to ensure the hardware, backend, and mobile app can communicate.

| Component | Target IP | File to Update | Variable/Line |
| :--- | :--- | :--- | :--- |
| **Backend API** | Laptop IP (`10.229.183.149`) | `Front-end/health_mate_app/lib/core/constants/api_constants.dart` | `devBaseUrl` (Android section) |
| **Hardware (ESP32)** | ESP32 IP (`10.229.183.78`) | `Back-end/app/core/config.py` <br> `Front-end/health_mate_app/lib/core/constants/iot_constants.dart` | `esp32_local_ip` <br> `esp32BaseUrl` |
...
3.  **⚠️ CONFIGURATION FOR EXTERNAL PHONE (IMPORTANT)**:
    If you are running the app on a **Real Physical Phone**, update the IP address:
    -   Open `lib/core/constants/api_constants.dart`.
    -   **Current Laptop IP**: `10.229.183.149`
    -   Change `devBaseUrl` to match your current network IP.
