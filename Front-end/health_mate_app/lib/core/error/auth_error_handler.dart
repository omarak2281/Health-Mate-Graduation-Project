...
class AuthErrorHandler {
  ...
  static String handleError(dynamic error) {
    ...
    final key = getErrorKey(error);
    return (key ?? 'errors.unknown_error').tr();
  }

  /// Extract a stable localization key for an auth error (no translation here)
  static String? getErrorKey(dynamic error) {
    // Firebase Auth Exceptions
    if (error is AuthException) {
      if (error.code == 'requires-recent-login') {
        return LocaleKeys.errorsRequiresRecentLogin;
      }
  ...
  static String _handleDioError(DioException error) {
    ...
    if (error.response != null) {
      ...
      if (data is Map && data.containsKey('detail')) {
        ...
        if (detail.contains('Firebase token verification failed') ||
            detail.contains('www.googleapis.com') ||
            detail.contains('NameResolutionError') ||
            detail.contains('Failed to resolve')) {
          return 'errors.network_error';
        }
