...
class _ECGHeartLineState extends State<ECGHeartLine>
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveWidth = widget.width > 0 ? widget.width.toDouble() : 200.0;
    final effectiveHeight = widget.height > 0 ? widget.height.toDouble() : 60.0;
...
class ECGPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // Safety check - don't paint if size is zero
    ...
    // Draw scanning effect with smooth easing
    final scanOffset = animationValue * w;

    // Add pulse effect that syncs with heartbeat
    final pulseScale = 1.0 + (math.sin(animationValue * math.pi * 4) * 0.05);
    final glowIntensity = 0.8 + (math.sin(animationValue * math.pi * 4) * 0.2);

    canvas.save();
    // Clip a "window" that moves with a glow - scaled by pulse
    final clipRect = Rect.fromLTWH(
      scanOffset - 60 * pulseScale,
      -10 * pulseScale,
      100 * pulseScale,
      h + 20 * pulseScale,
    );
    ...
    if (pathMetrics.isNotEmpty) {
      ...
      final bestTangent = metric.getTangentForOffset(bestLen);
      if (bestTangent != null) {
        dotOffset = bestTangent.position;
      }
    }
