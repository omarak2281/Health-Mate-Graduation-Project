...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  final _emailControllerFocusNode = FocusNode();
  ...
  void dispose() {
    ...
    _emailControllerFocusNode.dispose();
    super.dispose();
  }
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    try {
      ...
      if (errorKey == 'auth.google_not_registered') {
        if (!mounted) return;

        // Clear the error from UI
        ref.read(authNotifierProvider.notifier).clearError();

        // Show professional dialog with "Go to Register" option
        ProfessionalErrorHandler.showWarningDialog(
          context: context,
          titleKey: 'google_account_not_registered',
          messageKey:
              'This Google account is not registered in our system. Would you like to create a new account with this Google account?',
          ...
          onConfirm: () async {
            ...
            if (role != null && mounted) {
              // Navigate to RegisterPage with the selected role and Google signup flag
              Navigator.push(
                context,
                MaterialPageRoute(
      ...
      } else if (errorKey == 'auth.google_account_issue') {
        if (!mounted) return;

        ProfessionalErrorHandler.showErrorDialog(
          context: context,
          titleKey: 'google_account_issue',
          messageKey:
              'There\'s an issue with your Google account. Please try again or use a different sign-in method.',
          subtitleKey:
              'Make sure your Google account is active and has the necessary permissions.',
          showRetry: true,
          onRetry: _handleGoogleSignIn,
        );
      } else if (errorKey == 'auth.google_not_linked') {
        if (!mounted) return;

        ProfessionalErrorHandler.showWarningDialog(
          context: context,
          titleKey: 'google_account_not_linked',
          messageKey:
              'This email is registered but not linked to Google.\n\nChoose an option:\n• Sign in with your password and link Google in settings\n• Create a new account with Google',
          confirmText: 'Sign In with Password',
          cancelText: 'Create New Account',
          onConfirm: () {
            Navigator.of(context).pop();
            // Focus on email/password fields for manual login
            _emailController.clear();
            _passwordController.clear();
            FocusScope.of(context).requestFocus(_emailControllerFocusNode);
          },
          onCancel: () async {
            Navigator.of(context).pop();
            final role = await _showRoleSelectionDialog();
      ...
      } else if (errorKey == 'errors.email_already_in_use') {
        if (!mounted) return;

        ProfessionalErrorHandler.showErrorDialog(
          context: context,
          titleKey: 'account_already_exists',
          messageKey:
