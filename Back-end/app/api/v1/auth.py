...
from app.schemas.user import SocialAuthRequest, Token, UserLogin, UserRegister, UserResponse
...
async def social_auth(
    ...
    try:
        ...
        if not user:
            ...
            db.add(user)
            # We will update other fields below
            
        # 5. Always Update Profile Info (if provided in request or available from Google)
        if auth_data.phone:
            user.phone = auth_data.phone
        if auth_data.birth_date:
            user.birth_date = auth_data.birth_date
        if auth_data.gender:
            user.gender = auth_data.gender
        
        # Always update photo from Google if available
        if user_info.get('photo_url'):
            user.profile_image_url = user_info.get('photo_url')
            
        if email_verified:
            user.is_verified = True
            user.email_verified_at = datetime.utcnow()
        
        if user.auth_provider == 'email' or not user.auth_provider:
            user.auth_provider = user_info.get('auth_provider', 'google')

        await db.commit()
        await db.refresh(user)

        # Log event (Signup or Login)
        event_type = "user_registered" if auth_data.is_signup else "user_login"
        audit_log = AuditLog(
            user_id=user.id,
            event_type=event_type,
            event_description=f"User {event_type.replace('_', ' ')} via {user_info['auth_provider']}",
