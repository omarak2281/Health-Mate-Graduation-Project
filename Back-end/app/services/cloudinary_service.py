...
class CloudinaryService:
    ...
    async def upload_image(
        ...
        """
        Upload image to Cloudinary with production optimizations
        """
        if not all([self.cloud_name, self.api_key, self.api_secret]):
            raise HTTPException(
        ...
        try:
            ...
            # f_auto: optimize format (webp/avif)
            # q_auto: optimize quality
            # limit: resize if larger than 1000px while maintaining aspect ratio
            upload_result = cloudinary.uploader.upload(
                file_content,
                folder=f"health_mate/{folder}",
                public_id=public_id,
            ...
            raise HTTPException(
                ...
                detail="Image processing failed. Please check Cloudinary configuration."
            )
        finally:
            await file.seek(0)
