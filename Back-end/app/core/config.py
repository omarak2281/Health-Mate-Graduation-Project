...
# Get the project root directory (3 levels up from this file: app/core/config.py)
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent

# Docker paths (when running in container)
DOCKER_APP_ROOT = Path("/app")

# Check if running in Docker (Symptom-Checker folder exists at /app)
if (DOCKER_APP_ROOT / "Symptom-Checker").exists():
    MODELS_BASE_PATH = DOCKER_APP_ROOT
else:
    # Local development - project root is 3 levels up from this file
    MODELS_BASE_PATH = Path(__file__).resolve().parent.parent.parent.parent
...
class Settings(BaseSettings):
    ...
    max_drawers: int = 6
    esp32_local_ip: str = "10.229.183.78"
    esp32_activate_endpoint: str = "/activate"
